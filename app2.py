...
# --- Función auxiliar: centroid de GeoJSON Polygon/MultiPolygon ---
...
if st.session_state.step == 2:
    ...
    mapa_todas = folium.Map(location=[4.624335, -74.063644], zoom_start=11)
...
# 3.1 Obtener centro dinámico de la localidad
feat_sel    = next(
    feat for feat in localidades_geo["features"]
    if feat["properties"]["nombre_localidad"] == sel
)
centro_loc  = get_centroid(feat_sel)

st.header(f"📍 Paso 3: Mapa de {sel}")
mapa_sel = folium.Map(location=centro_loc, zoom_start=11)
for feat in localidades_geo["features"]:
...
codigo_loc = feat_sel["properties"]["num_localidad"]
pot_local  = [
    feat for feat in pot_geo["features"]
    if feat["properties"]["num_localidad"] == codigo_loc
]

# 3.4 Mapa POT con paleta de colores y centrado dinámico
cats      = []
...
mapa_pot = folium.Map(location=centro_loc, zoom_start=11)
for feat in pot_local:
    uso = feat["properties"]["uso_pot_simplificado"]
    col = color_map[uso]
    folium.GeoJson(
